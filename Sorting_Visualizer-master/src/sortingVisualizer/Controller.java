...
public final class Controller implements Initializable {
    ...
    private Label statusLabel, arraySizeSliderLabel, delaySliderLabel,
            ...
            linearSearchBtn, binarySearchBtn;
    @FXML
    ...
    private final String BACKGROUND_THEME = "-fx-background-color: #0C2633;",
            ...
            FOUND_COLOR = "-fx-background-color: #39FF14;",
            NOT_FOUND_COLOR = "-fx-background-color: #FF0000;";
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        linearSearchBtn.setOnMouseClicked(e -> linearSearch());
        binarySearchBtn.setOnMouseClicked(e -> binarySearch());

        aboutBtn.setOnMouseClicked(e -> {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("About Application & Developer");
            alert.setHeaderText("Developed by Team ALGOVIZ");
    ...
    private void setAllDisable(boolean b) {
        ...
        linearSearchBtn.setDisable(b);
        binarySearchBtn.setDisable(b);
    }
        ...
        try {
            ...
            new Thread(() -> {
                ...
                for (int i = 0; i < TOTAL_NUMBER_OF_BARS; i++) {
                    ...
                    changeStyleEffect(currentIndex, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
                    delay();

                    if ((int) ((XYChart.Data) series.getData().get(currentIndex)).getYValue() == searchValue) {
                        found = true;
                        changeStyleEffect(currentIndex, FOUND_COLOR);
                        Platform.runLater(() -> {
                            statusLabel.setText("FOUND at index: " + currentIndex);
                            statusLabel.setStyle(TEXT_COLOR_BRIGHT_GREEN);
                        });
                        break;
                    }

                    changeStyleEffect(currentIndex, MAIN_THEME);
                }

                if (!found) {
                    Platform.runLater(() -> {
                        statusLabel.setText("NOT FOUND");
                        statusLabel.setStyle(NOT_FOUND_COLOR);
                    });
                }
    ...
    private void binarySearch() {
        try {
            ...
            new Thread(() -> {
                ...
                int left = 0;
                int right = TOTAL_NUMBER_OF_BARS - 1;
                boolean found = false;
                ...
                while (left <= right) {
                    ...
                    for (int i = left; i <= right; i++) {
                        changeStyleEffect(i, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
                    }
                    changeStyleEffect(mid, CURRENT_INDEX_COLOR);
                    delay();

                    int midValue = (int) ((XYChart.Data) series.getData().get(mid)).getYValue();

                    if (midValue == searchValue) {
                        found = true;
                        changeStyleEffect(mid, FOUND_COLOR);
                        Platform.runLater(() -> {
                            statusLabel.setText("FOUND at index: " + mid);
                            statusLabel.setStyle(TEXT_COLOR_BRIGHT_GREEN);
                        });
                        break;
                    }
                    ...
                    // Reset colors
                    for (int i = 0; i < TOTAL_NUMBER_OF_BARS; i++) {
                        changeStyleEffect(i, MAIN_THEME);
                    }
                }
